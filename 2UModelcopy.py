import numpy as np
import constants_2U as constants
import qnv as qnv
...
def aero_torque(s):
    ...
    area = l2 * l3 * abs(cosw1) + l1 * l3 * abs(cosw2) + l1 * l2 * abs(
        cosw3)  # area of satellite perpendicular to velocity
...
def solar_torque(s):
    ...
    v_t_sd2_b = np.cross(r_com, [l2*l3*abs(cosw1)*cosw1, l1*l3*abs(cosw2)*cosw2, l1*l2*abs(cosw3)*cosw3])*2*e*p
    # reflection torque
